from datetime import date, datetime, timedelta

from trading.backtest.models import BacktestConfig
from trading.backtest.validation import run_out_of_sample_validation
from trading.risk.models import RiskConfig
from trading.signals.models import Candle

BASE_DAY = date(2026, 8, 10)
TRADING_DAYS = [BASE_DAY + timedelta(days=i) for i in range(10)]  # index 0 -> train, index 8 -> test


def candle(o, h, l, c, minute, v=1000):
    return Candle(time=datetime(2026, 8, 14, 9, 30 + minute), open=o, high=h, low=l, close=c, volume=v)


class FakeDataSource:
    def __init__(self, premarket, session):
        self._premarket = premarket
        self._session = session

    def get_premarket_candles(self, symbol, day):
        return self._premarket.get((symbol, day), [])

    def get_session_candles(self, symbol, day, end_time=None):
        return self._session.get((symbol, day), [])


def flat_session():
    # A spike candle sets the day's high to 5.60, giving the following
    # day's setup a prior-day-high target that clears the minimum R:R.
    return [candle(5.00, 5.02, 4.98, 5.00, i) for i in range(9)] + [candle(5.00, 5.60, 4.98, 5.02, 9)]


def long_setup_session():
    return [
        candle(4.70, 4.72, 4.68, 4.71, 0),
        candle(4.71, 4.73, 4.69, 4.72, 1),
        candle(4.72, 4.74, 4.70, 4.73, 2),
        candle(4.73, 4.75, 4.71, 4.74, 3),
        candle(4.74, 4.76, 4.72, 4.75, 4),
        candle(4.75, 4.76, 4.74, 4.75, 5),
        candle(4.75, 4.76, 4.74, 4.75, 6),
        candle(4.75, 4.76, 4.74, 4.75, 7),
        candle(4.75, 4.76, 4.55, 4.70, 8),
        candle(4.95, 5.20, 4.90, 5.15, 9),
        candle(5.15, 5.25, 4.85, 5.20, 10),
        candle(5.20, 5.30, 5.15, 5.25, 11),
        candle(5.25, 5.65, 5.20, 5.60, 12),
    ]


def make_config():
    return BacktestConfig(
        risk_config=RiskConfig(risk_pct_per_trade=0.005, min_risk_reward=2.0, atr_stop_multiplier=1.0),
        starting_equity=10_000,
        opening_range_minutes=5,
        atr_period=14,
        min_target_distance=0.10,
    )


def test_validation_isolates_trades_to_their_split():
    # Each split is backtested independently (no prior-day context carried
    # in from outside the split), so the special day can't be the first day
    # of its split - it needs a flat/spike predecessor within the same split
    # to supply the prior-day-high target. train = indices 0-5, test = 8-9.
    train_day = TRADING_DAYS[1]
    test_day = TRADING_DAYS[9]

    session = {("ABCD", day): flat_session() for day in TRADING_DAYS}
    session[("ABCD", train_day)] = long_setup_session()
    session[("ABCD", test_day)] = long_setup_session()

    premarket = {
        ("ABCD", train_day): [candle(4.70, 4.72, 4.65, 4.68, -30)],
        ("ABCD", test_day): [candle(4.70, 4.72, 4.65, 4.68, -30)],
    }

    data_source = FakeDataSource(premarket, session)

    report = run_out_of_sample_validation(["ABCD"], TRADING_DAYS, data_source, make_config())

    assert report.train.total_trades == 1
    assert round(report.train.total_pnl, 2) == 140.00
    assert report.validate.total_trades == 0
    assert report.test.total_trades == 1
    assert round(report.test.total_pnl, 2) == 140.00


def test_validation_with_no_signals_anywhere():
    session = {("ABCD", day): flat_session() for day in TRADING_DAYS}
    data_source = FakeDataSource(premarket={}, session=session)

    report = run_out_of_sample_validation(["ABCD"], TRADING_DAYS, data_source, make_config())

    assert report.train.total_trades == 0
    assert report.validate.total_trades == 0
    assert report.test.total_trades == 0
